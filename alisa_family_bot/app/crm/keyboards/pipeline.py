...
def get_thinking_keyboard(page: int, has_prev: bool, has_next: bool, sort_by: str) -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = [
        [InlineKeyboardButton(text="▶️ Почати прозвон", callback_data=f"thinking:open:{page}:{sort_by}")],
        [
            InlineKeyboardButton(text="↩️ В очередь", callback_data=f"thinking:action:back:{page}:{sort_by}"),
            InlineKeyboardButton(text="❌ Отказ", callback_data=f"thinking:action:reject:{page}:{sort_by}"),
        ],
    ]
    nav_row: list[InlineKeyboardButton] = []
    if has_prev:
        nav_row.append(InlineKeyboardButton(text="⬅️", callback_data=f"thinking:page:{page - 1}:{sort_by}"))
    if has_next:
        nav_row.append(InlineKeyboardButton(text="➡️", callback_data=f"thinking:page:{page + 1}:{sort_by}"))
    if nav_row:
        rows.append(nav_row)
